    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50">
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-slate-600 mt-2">Complete information about this project</p>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-8">
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="text-slate-900">{project.propertyType || 'N/A'}</p>
              </div>
              <div>
                <label className="text-sm font-medium text-slate-600">Location</label>
                <p className="text-slate-900">{project.location || project.city || 'N/A'}</p>
              </div>
              <div>
                <label className="text-sm font-medium text-slate-600">Procurement Method</label>
                <p className="text-slate-900">{project.procurementMethod || 'N/A'}</p>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="text-sm font-medium text-slate-600">Project Duration</label>
                <p className="text-slate-900">{project.estimatedDuration || 'N/A'}</p>
              </div>
              <div>
                <label className="text-sm font-medium text-slate-600">Tender Duration</label>
                <p className="text-slate-900">{project.tenderDuration || 'N/A'}</p>
              </div>
              <div>
                ...
                <p className="text-slate-900">
                  {project.createdAt ? (
                    project.createdAt.toDate ? 
                      new Date(project.createdAt.toDate()).toLocaleDateString('id-ID', {
                        year: 'numeric',
                        month: 'long',
                        day: 'numeric'
                      }) :
            ...
            <div className="space-y-4">
              <h3 className="text-lg font-semibold text-slate-900 border-b border-slate-200 pb-2">Vendor Information</h3>
              <div>
                <label className="text-sm font-medium text-slate-600">Awarded Vendor</label>
                <p className="text-slate-900">{project.awardedVendor || 'Not awarded yet'}</p>
              </div>
              <div>
                <label className="text-sm font-medium text-slate-600">Vendor Email</label>
                <p className="text-slate-900">{project.awardedVendorEmail || 'N/A'}</p>
              </div>
              <div>
                <label className="text-sm font-medium text-slate-600">Proposals Received</label>
                <p className="text-slate-900">{project.proposals?.length || 0} proposals</p>
              </div>
              <div>
                <label className="text-sm font-medium text-slate-600">Final Price</label>
                <p className="text-slate-900">{project.finalPrice ? formatCurrency(project.finalPrice) : 'Not set'}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        {project.proposals && project.proposals.length > 0 && (
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-8 mb-6">
            <h3 className="text-xl font-bold text-slate-900 mb-6">Proposals Summary</h3>
            <div className="space-y-4">
              {project.proposals.map((proposal, index) => (
                <div key={index} className="flex items-center justify-between p-4 bg-slate-50 rounded-lg">
                  <div>
                    <h4 className="font-medium text-slate-900">{proposal.vendorName || proposal.vendorEmail}</h4>
                    <p className="text-sm text-slate-600">
                      Submitted: {proposal.createdAt ? 
                        new Date(proposal.createdAt.toDate ? proposal.createdAt.toDate() : proposal.createdAt).toLocaleDateString('id-ID') 
                        : 'N/A'
                      }
                    </p>
                  </div>
                  <div className="text-right">
                    <p className="font-bold text-slate-900">{formatCurrency(proposal.proposedPrice)}</p>
        ...
        {project.milestones && project.milestones.length > 0 && (
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-8 mb-6">
            <h3 className="text-xl font-bold text-slate-900 mb-6">Project Milestones</h3>
            <div className="space-y-4">
              {project.milestones.map((milestone, index) => (
                <div key={index} className="flex items-center space-x-4 p-4 bg-slate-50 rounded-lg">
                  ...
                  <span className={`px-3 py-1 rounded-full text-sm font-medium ${
                    milestone.completed ? 'bg-green-100 text-green-800' : 'bg-slate-100 text-slate-600'
                  }`}>
        ...
        {project.attachedBOQ && (
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-8 mb-6">
            <h3 className="text-xl font-bold text-slate-900 mb-6">Bill of Quantities (BOQ)</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label className="text-sm font-medium text-slate-600">Total Items</label>
                <p className="text-slate-900">{project.attachedBOQ.items?.length || 0} items</p>
              </div>
              <div>
                <label className="text-sm font-medium text-slate-600">Total BOQ Value</label>
                <p className="text-slate-900">{formatCurrency(project.attachedBOQ.totalCost)}</p>
              </div>
            </div>
          </div>
        )}
