...
export default function TransactionDetailPage() {
    ...
    const fetchTransaction = async () => {
      ...
      try {
        ...
        if (transactionDoc.exists()) {
          ...
          // Verify this transaction belongs to the current vendor
          if (transactionData.vendorId === user.uid || transactionData.vendorEmail === user.email) {
  ...
  const getStatusDescription = (status) => {
    switch (status) {
      ...
      case 'release':
        return 'Funds being released';
      case 'settle':
        return 'Payment received';
      case 'add-funds':
        return 'Additional funds requested';
      case 'refund':
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="text-slate-900">{transaction.clientName}</p>
              </div>
              <div>
                <label className="text-sm font-medium text-slate-600">Client Email</label>
                <p className="text-slate-900">{transaction.clientEmail}</p>
              </div>
              <div>
