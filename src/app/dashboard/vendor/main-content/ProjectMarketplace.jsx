...
const ProjectMarketplace = () => {
      ...
      snapshot.forEach((doc) => {
        ...
        if (
          ...
          if (data.proposals && Array.isArray(data.proposals)) {
            hasAcceptedProposal = data.proposals.some(
              (proposal) =>
          ...
          if (finalDeadline) {
            ...
            try {
              if (finalDeadline instanceof Date) {
                ...
                typeof finalDeadline === "object" &&
                finalDeadline.seconds
              ) {
              ...
              if (deadlineDate && !isNaN(deadlineDate.getTime())) {
                ...
                if (diffTime <= 0) {
                  console.log(
                    `Skipping project ${doc.id} - deadline has passed:`,
                    deadlineDate
                  );
                  return;
                }
                ...
                if (hoursToDeadline <= 24) {
                  console.log(
                    `Skipping project ${
                      doc.id
                    } - project is locked (${hoursToDeadline.toFixed(
                      1
                    )} hours to deadline):`,
                    deadlineDate
                  );
              ...
              console.error(
                `Error checking deadline for project ${doc.id}:`,
                error
              );
  ...
  const filteredData = marketData.filter((item) => {
    ...
    const timeToDeadline = getTimeToDeadlineInHours(
      item.deadline || item.tenderDeadline
    );
  ...
  const handleCreateProposal = (project) => {
    ...
    if (project.isResubmission) {
      console.log(
        "🔄 Opening proposal editor for resubmission:",
  ...
  const hasVendorSubmittedProposal = (project) => {
    ...
    return normalizedProposals.some(
  ...
  const canEditProposal = (project) => {
    ...
    if (project.deadline) {
      ...
      if (project.tenderDeadline instanceof Date) {
        ...
        typeof project.tenderDeadline === "object" &&
        project.tenderDeadline.seconds
      ) {
    ...
    <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      {loading ? (
        ...
        <CreateProposalPage
          ...
          existingProposal={
            selectedProject.isResubmission
              ? selectedProject.existingProposal
              : getVendorProposal(selectedProject)
          }
          isEditing={
            selectedProject.isResubmission ||
            hasVendorSubmittedProposal(selectedProject)
          }
                  ...
                  <div className="p-6">
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-6">
                      ...
                      <div>
                        ...
                        <p className="text-sm font-semibold text-gray-900">
                          {getProposalsLength(project.proposals)} terkirim
                        </p>
                      </div>
                    </div>
                    {/* Actions */}
