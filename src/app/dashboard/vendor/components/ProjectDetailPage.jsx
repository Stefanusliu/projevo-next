...
export default function ProjectDetailPage({
  ...
  const hasAcceptedProposal = () => {
    ...
    const isAccepted = normalizedProposals.some(
      (proposal) =>
  ...
  const getDynamicTabs = () => {
    ...
    console.log(
      "ProjectDetailPage (Vendor) - payments array:",
      project?.payments
    );
    console.log(
      "ProjectDetailPage (Vendor) - payment object:",
      project?.payment
    );
  ...
  const getVendorNegotiationStatus = () => {
    ...
    return {
      ...
      canVendorAct:
        negotiationData?.status === "pending_vendor_response" ||
  ...
  const getVendorProposalIndex = () => {
    ...
    const proposalIndex = normalizedProposals.findIndex(
      (proposal) =>
        proposal.vendorId === user.uid ||
        proposal.userId === user.uid ||
        proposal.submittedBy === user.uid
  ...
  const startNegotiation = () => {
    if (pendingCounterOffer) {
      alert(
        "You have already submitted a counter offer. Please wait for the project owner to respond."
      );
      return;
    }
    ...
    const vendorProposal =
      proposalIndex >= 0 ? normalizedProposals[proposalIndex] : null;
    ...
    if (vendorProposal?.boqPricing) {
      ...
      vendorProposal.boqPricing.forEach((item, index) => {
        ...
        const currentPrice =
          existingOffer?.vendorPrice !== undefined
  ...
  const submitCounterOffer = async () => {
    ...
    try {
      ...
      const existingNegotiation = currentProposal.negotiation || {
        history: [],
      };
      ...
      const updatedNegotiation = {
        ...
        vendorInfo: {
          vendorId: user.uid,
          vendorName:
            userProfile?.accountType === "perusahaan"
              ? userProfile?.company ||
                user.displayName ||
                user.email?.split("@")[0] ||
                "Unknown Vendor"
        ...
        history: [...(existingNegotiation.history || []), newHistoryEntry],
        updatedAt: new Date(),
      };
      ...
      alert(
        "Counter offer submitted successfully! Please wait for the project owner to respond."
      );
    } catch (error) {
  ...
  const handleResubmitProposal = () => {
    try {
      ...
      router.push(
        `/boq-penawaran?projectId=${project.id}&proposalId=${
          currentProposal.id || currentProposal.proposalId
        }`
      );
            ...
            jenis.uraianKerja.forEach((uraian, uraianIndex) => {
              if (uraian.items) {
                uraian.items.forEach((item, itemIndex) => {
                  ...
                  const flatItemIndex = calculateFlatItemIndex(
                    updatedBOQ.tahapanKerja,
                    tahapanIndex,
                    jenisIndex,
                    uraianIndex,
                    itemIndex
                  ...
                  // Check for negotiated price in negotiation data
                    ...
                    negotiationData.counterOffer[flatItemIndex]
                  ) {
                    negotiatedPrice = parseFloat(
                      negotiationData.counterOffer[flatItemIndex].vendorPrice ||
                        item.unitPrice
                    );
                  }
  ...
  const enhancedProject = {
    ...project,
    projectTitle:
      project.name || project.title || "Project Title Not Available",
    province:
      project.province ||
    ...
    fullAddress:
      project.address || project.fullAddress || "Address Not Available",
    clientName:
      project.client || project.createdBy || "Client Name Not Available",
  ...
  const handleUploadDocumentation = async (files) => {
    if (!isProjectInProgress()) {
      alert(
        "Documentation upload is only available when the project is in progress."
      );
      return;
    }
    ...
    try {
      ...
      console.log(
        "✅ Documentation images uploaded successfully:",
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8 space-y-8">
                  ...
                  >
                    <div
                      className={`w-3 h-3 rounded-full transition-all duration-300 ${
                        index <
                        tahapanList.length * ((project.match || 0) / 100)
                          ? "bg-blue-600"
                          : "bg-gray-300"
                      }`}
                    ></div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div>
              ...
              <p className="text-gray-700 font-bold">
                {enhancedProject.projectType || "Not specified"}
              </p>
            </div>
            <div>
              <h3 className="text-lg font-normal text-gray-900 mb-2">
              ...
              <p className="text-gray-700 font-bold">
                {enhancedProject.propertyType || "Not specified"}
              </p>
            </div>
          </div>
              ...
              <div className="flex flex-wrap gap-2">
                {Array.isArray(project.projectScope) ? (
                  project.projectScope.map((scope, index) => (
                    <span
                      key={index}
                      className="px-3 py-1 bg-blue-100 text-blue-800 rounded-lg text-sm font-bold"
                    >
                      {scope}
                    </span>
                  ))
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            ...
            <div>
              ...
              <p className="text-gray-900 font-bold">
                {formatDate(enhancedProject.estimatedStartDate) ||
                  "Not specified"}
              </p>
            </div>
            <div>
            ...
            <div className="flex items-start space-x-3">
              ...
              <div>
                <h5 className="font-medium text-green-800 mb-1">
                  Project Accepted - Work Started
                </h5>
                <p className="text-sm text-green-700">
                  You have been selected for this project. The BOQ below shows
                  the agreed terms for your work.
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="flex items-start space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-blue-700">
                  The BOQ below shows the latest negotiated prices from your
          ...
          <div className="w-full overflow-hidden">
            <BOQDisplay
              project={{
                ...project,
                ...(project.attachedBOQ && {
                  attachedBOQ: calculateLatestNegotiatedPrices(
                    project.attachedBOQ
                  ),
                }),
                ...(project.boq && {
                ...
                ...(project.tahapanKerja && {
                  tahapanKerja: calculateLatestNegotiatedPrices({
          ...
          {isVendorAccepted && (
            <div className="mt-4 p-4 bg-gray-50 rounded-lg">
              <div className="flex items-center space-x-2 text-sm text-gray-600">
                ...
                <span>
                  This is your accepted work scope. Use the Documentation tab to
  ...
  const renderProposalTab = () => {
          ...
          <div className="flex items-start space-x-3">
            ...
            <div>
              ...
              <p className="text-yellow-700 mb-4">
                You haven't submitted a proposal for this project yet. Click the
                button below to create your proposal.
              </p>
              <button
    ...
    const currentStatus =
      negotiationData?.status || vendorProposal.status || "pending";
      ...
      vendorProposal.boqPricing.forEach((item, index) => {
        ...
        if (isNegotiating && tempCounterOffers[index]) {
          currentPrice = parseFloat(
            tempCounterOffers[index].vendorPrice || originalPrice
          );
    ...
    const hasNegotiatedPrices = Object.keys(counterOffers).some((key) =>
      key.startsWith(`${proposalIndex}_`)
    );
    ...
    return (
      <div className="space-y-6">
            ...
            <div className="flex items-start space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-yellow-700">
                  Your counter offer has been submitted and is awaiting the
                  project owner's response. You cannot submit another counter
                  offer until they respond.
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <div className="flex items-start justify-between mb-6">
            <div>
              ...
              <div className="flex items-center gap-3">
                <span
                  className={`px-3 py-1 text-sm font-medium rounded-full ${getProposalStatusColor(
                    currentStatus
                  )}`}
                >
                ...
                <span className="text-gray-500">
                  Submitted{" "}
                  {vendorProposal.submittedAt
                    ? new Date(vendorProposal.submittedAt).toLocaleDateString(
                        "id-ID",
                        {
                          year: "numeric",
                          month: "long",
            ...
            <div className="text-right">
              ...
              <p
                className={`text-2xl font-bold ${
                  isNegotiating ? "text-gray-900" : "text-blue-600"
                }`}
              >
                {formatCurrency(latestTotal)}
              </p>
              {hasNegotiatedPrices &&
                originalTotal !== latestTotal &&
                !isNegotiating && (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            ...
            <div className="bg-gray-50 p-4 rounded-lg">
              ...
              <p className="font-semibold text-gray-900">
                {vendorProposal.negotiable === "negotiable"
                  ? "Negotiable"
                  : "Fixed Price"}
              </p>
            </div>
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between items-start mb-1">
                      <p className="font-medium text-gray-900">
                        {entry.action === "counter_offer_sent" &&
              ...
              <div className="bg-blue-50 border border-blue-200 p-4 rounded-lg mb-4">
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-blue-700">
                      Some items have been negotiated. Updated prices are shown
                      below.
                    </p>
                  </div>
                </div>
                  ...
                  {vendorProposal.boqPricing.map((item, index) => {
                    ...
                    return (
                      <tr
                        key={index}
                        className={
                          hasNegotiation && !isNegotiating ? "bg-blue-50" : ""
                        }
                      >
                          ...
                          {isNegotiating ? (
                            <div className="space-y-1">
                              <input
                                ...
                                step="any"
                                value={
                                  tempCounterOffers[index]?.vendorPrice ||
                                  currentPrice
                                }
                                onChange={(e) =>
                                  updateTempCounterOffer(
                                    index,
                                    "vendorPrice",
                                    e.target.value
                                  )
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span
                            className={`text-sm font-medium ${
                              hasNegotiation && !isNegotiating
                                ? "text-blue-600"
                                : "text-gray-900"
                            }`}
                          >
                            {formatCurrency(
                              currentPrice * (item.volume || item.quantity || 0)
                            )}
                          </span>
                        </td>
                      </tr>
        ...
        {(currentStatus === "negotiating" ||
          currentStatus === "pending_vendor_response") && (
          <div className="bg-white rounded-lg border border-gray-200 p-6">
            <h3 className="text-lg font-bold text-gray-900 mb-4">
              {currentStatus === "negotiating"
                ? "Negotiation Request"
              ...
              <div className="space-y-4">
                <div className="bg-blue-50 border border-blue-200 p-4 rounded-lg">
                  <div className="flex items-start space-x-3">
                    ...
                    <div>
                      <h5 className="font-medium text-blue-800 mb-1">
                        Project Owner Requests Negotiation
                      </h5>
                      <p className="text-sm text-blue-700">
                        The project owner would like to negotiate terms. You can
                        resubmit your proposal with updated pricing or decline
                        the negotiation.
                      </p>
                    </div>
                  </div>
                ...
                <div className="flex space-x-3">
                  ...
                  <button
                    onClick={() => {
                      console.log(
                        "🔘 Decline negotiation button clicked (vendor)"
                      );
              ...
              <div className="flex space-x-3">
                <button
                  ...
                  className={`px-6 py-3 bg-gradient-to-r from-green-600 to-green-700 text-white rounded-lg font-medium hover:from-green-700 hover:to-green-800 transition-all duration-200 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 ${
                    pendingCounterOffer ? "opacity-50 cursor-not-allowed" : ""
                  }`}
                >
      ...
      return (
        <div className="bg-white rounded-lg border border-gray-200 p-8 text-center">
          ...
          <p className="text-gray-600">
            Documentation upload will be available once the project is in
            progress.
          </p>
        </div>
      );
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-2">
              <input
                ...
                accept="image/*"
                onChange={(e) =>
                  e.target.files && handleUploadDocumentation(e.target.files)
                }
                className="hidden"
          ...
          <p className="text-sm text-gray-600">
            Upload images to document your work progress. Supported formats:
            JPG, PNG, GIF. Maximum size: 10MB per image.
          </p>
        </div>
          ...
          <div className="w-full px-4 sm:px-6 lg:px-8">
            <div className="flex items-center justify-between h-16">
              ...
              <div className="text-center flex-1">
                ...
                <p className="text-sm text-gray-500">
                  Detailed BOQ View • Full Screen{" "}
                  {Object.keys(negotiations).length > 0
                    ? "• Latest Negotiated Prices"
                    : ""}
                </p>
              </div>

              <div className="text-sm text-gray-500">Vendor Dashboard</div>
            </div>
          </div>
        </div>
              ...
              <div className="bg-blue-50 border border-blue-200 p-4 rounded-lg mb-6">
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-blue-700">
                      This BOQ shows the latest negotiated prices from your
                      discussions with the project owner. Items with negotiated
                      prices are highlighted with the latest agreed amounts.
                    </p>
                  </div>
                </div>
              </div>
          ...
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center space-x-4">
              ...
              {(() => {
                const proposalIndex = getVendorProposalIndex();
                const vendorProposal =
                  proposalIndex >= 0 ? project.proposals[proposalIndex] : null;
                const negotiationData = negotiations[proposalIndex];
                const currentStatus =
                  negotiationData?.status ||
                  vendorProposal?.status ||
                  "pending";
                  ...
                  return (
                    <div className="flex space-x-2">
                      <button
                        onClick={() => {
                          console.log(
                            "🔘 Resubmit button clicked (vendor - mobile)"
                          );
                          handleResubmitProposal();
                        }}
                      ...
                      <button
                        onClick={() => {
                          console.log(
                            "🔘 Decline negotiation button clicked (vendor - mobile)"
                          );
                          handleDeclineNegotiation();
                        }}
                  ...
                  return (
                    <div className="flex space-x-2">
                      <button
                        onClick={() => {
                          console.log(
                            "🔘 Accept Terms button clicked (vendor - small)"
                          );
                          handleAcceptNegotiation();
                        }}
                        className="px-4 py-2 bg-green-600 text-white rounded-lg font-medium hover:bg-green-700 transition-colors text-sm"
