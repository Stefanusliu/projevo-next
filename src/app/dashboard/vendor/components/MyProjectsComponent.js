...
export default function MyProjectsComponent({ projectFilter = "All" }) {
  ...
  const handleCreateProposal = (project) => {
    ...
    if (project.isResubmission && project.existingProposal) {
      console.log(
        "🔄 Opening BOQ penawaran for resubmission:",
        project.existingProposal
      );
      // Navigate to BOQ-penawaran page with project and proposal data
      router.push(
        `/boq-penawaran?projectId=${project.id}&proposalId=${project.existingProposal.id}`
      );
    } else {
  ...
  const handleResubmitProposal = (project) => {
    ...
    const vendorProposal = project.proposals?.find(
    ...
    if (vendorProposal) {
      ...
      router.push(
        `/boq-penawaran?projectId=${project.id}&proposalId=${vendorProposal.id}`
      );
    } else {
      ...
      (snapshot) => {
        ...
        allProjects.forEach((project) => {
          if (project.proposals && project.proposals.length > 0) {
            console.log(
              `Project ${project.id}: procurementMethod = ${project.procurementMethod}, hasProposals = ${project.proposals.length}`
            );
          }
        });
        ...
        const vendorProjects = allProjects.filter((project) => {
          ...
          if (project.proposals && project.proposals.length > 0) {
            console.log(
              `Project ${project.id} has proposals:`,
              project.proposals
            );
            project.proposals.forEach((proposal, index) => {
              console.log(
                `  Proposal ${index}: vendorId = ${proposal.vendorId}, current user = ${user.uid}`
              );
            });
          }
          ...
          const hasProposal = project.proposals?.some((proposal) => {
            console.log(
              `Checking proposal: ${proposal.vendorId} === ${user.uid} = ${
          ...
          console.log(
            `Project ${project.id}: hasProposal = ${hasProposal}, isAwarded = ${isAwarded}`
          );
        ...
        vendorProjects.forEach((project) => {
          console.log(
            `Vendor project ${project.id}: procurementMethod = "${project.procurementMethod}"`
          );
        });
  ...
  if (projectFilter === "Tender") {
    typeFilteredProjects = projects.filter(
      (project) => project.procurementMethod === "Tender"
    );
    console.log("Filtered for Tender projects:", typeFilteredProjects.length);
  } else if (projectFilter === "Contract") {
    typeFilteredProjects = projects.filter(
      (project) => project.procurementMethod === "Contract"
    );
  ...
  const getProjectLocation = (project) => {
    return (
      project.marketplace?.location?.city || project.city || "Unknown Location"
    );
  };
  ...
  const getProjectDescription = (project) => {
    return (
      ...
      project.description ||
      `${project.projectType} project for ${
        project.propertyType || "property"
      } in ${getProjectLocation(project)}`
    );
  };
  ...
  const getVendorStatus = (project) => {
    ...
    if (vendorProposal) {
      ...
      const hasRecentUpdate =
        vendorProposal.hasRecentUpdate ||
              ...
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="text-2xl font-bold text-slate-900">
                    {projectFilter === "All" || projectFilter === "Project"
                      ? "Semua Proyek"
                      : projectFilter === "Tender"
                      ? "Proyek Tender"
                      : "Proyek Kontrak"}
                  </h2>
                  <p className="text-slate-600">
                    {projectFilter === "All" || projectFilter === "Project"
                      ? "Lihat dan lacak proyek yang Anda terima"
                      : projectFilter === "Tender"
                      ? "Lihat dan lacak proyek tender Anda"
