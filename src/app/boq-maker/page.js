...
function BOQMakerWrapper() {
  const { user, userProfile, loading } = useAuth();
  const router = useRouter();

  useEffect(() => {
    console.log("BOQ Maker - Auth state:", {
      user: !!user,
    ...
    if (!loading && !user) {
      console.warn("⚠️ No user found after loading completed");
      // Redirect to login after a short delay to allow for potential auth state updates
      setTimeout(() => {
        router.push("/login");
      }, 2000);
    }
      ...
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-4">
            BOQ Studio hanya tersedia untuk Pemilik Proyek.
          </p>
          <button
  ...
  // Render the actual BOQ Maker component once authenticated
  return (
    <BOQMakerComponent
      user={user}
      userProfile={userProfile}
...
// Main BOQ Maker component
function BOQMakerComponent({ user, userProfile, loading }) {
  const router = useRouter();
  const searchParams = useSearchParams();

  // Helper function to convert boqPricing array to tahapanKerja structure
  const convertBoqPricingToTahapanKerja = (
    boqPricing,
    ...
    boqPricing.forEach((item, index) => {
      const tahapanName =
        item.tahapanName || item.tahapanKerja || "Tahapan Kerja Utama";
      ...
      if (
        ...
        negotiationData.counterOffer[index]
      ) {
      ...
      const originalPrice = item.hasOriginalPrice
        ? parseFloat(item.originalPrice || 0)
        : 0;
      const currentPrice = parseFloat(
        item.currentPrice || item.vendorPrice || item.subtotal || 0
      );
      ...
      groupedData[tahapanName][jenisName].push({
        ...
        specs: [
          {
            description:
              item.item || item.description || item.name || uraianName,
    ...
    const tahapanKerja = Object.keys(groupedData).map((tahapanName) => {
      const jenisKerja = Object.keys(groupedData[tahapanName]).map(
        (jenisName) => {
          const uraian = groupedData[tahapanName][jenisName].map(
            (uraianItem) => {
  ...
  // All React hooks must be declared at the top level, before any conditional returns
  ...
  useEffect(() => {
    ...
    if (!loading && user && userProfile) {
      ...
      if (userProfile.userType !== "project-owner") {
        console.log(
          "BOQ Maker - Redirecting non-project-owner:",
  ...
  const loadTempBOQData = useCallback(
    async (tempId, isReadOnly) => {
      try {
        console.log("📥 Loading temporary BOQ data from Firestore...", tempId);

        const tempBOQData = await firestoreService.get(
          "temp_boq_views",
          tempId
        );
        ...
        if (!tempBOQData) {
          console.warn(
            "⚠️ Temporary BOQ data not found in Firestore, checking localStorage fallback..."
          );

          // Try to find a fallback in localStorage using a pattern
          const localStorageKeys = Object.keys(localStorage);
          const boqKey = localStorageKeys.find(
            (key) =>
              key.startsWith("temp_boq_") && localStorage.getItem(key) !== null
          );

          if (boqKey) {
            console.log(
              "🔄 Found localStorage fallback, switching to local data..."
            );
          ...
          throw new Error(
            "Temporary BOQ data not found in Firestore or localStorage. The data may have expired or been removed."
          );
        }
        ...
        if (tempBOQData.boqPricing && Array.isArray(tempBOQData.boqPricing)) {
          ...
          const convertedTahapanKerja = convertBoqPricingToTahapanKerja(
            tempBOQData.boqPricing
          );

          setBoqTitle(
            `${tempBOQData.vendorName} - ${tempBOQData.projectTitle} BOQ`
          );
          ...
          setTimeout(async () => {
            try {
              ...
              console.warn(
                "⚠️ Failed to cleanup temporary data:",
                cleanupError
              );
            }
          ...
          throw new Error(
            "BOQ data structure is invalid or missing pricing information"
          );
        }
      } catch (error) {
  ...
  const loadLocalBOQData = (localKey, isReadOnly) => {
    try {
      console.log(
        "📥 Loading temporary BOQ data from localStorage...",
        localKey
      );
      ...
      if (!tempBOQDataString) {
        console.warn(
          "⚠️ Temporary BOQ data not found in localStorage with key:",
          localKey
        );

        // Try to find any temporary BOQ data as fallback
        const localStorageKeys = Object.keys(localStorage);
        const allBoqKeys = localStorageKeys.filter((key) =>
          key.startsWith("temp_boq_")
        );

        if (allBoqKeys.length > 0) {
          console.log(
            "🔄 Found alternative BOQ data, using most recent...",
            allBoqKeys
          );
          ...
          if (fallbackDataString) {
            ...
            if (
              fallbackData.boqPricing &&
              Array.isArray(fallbackData.boqPricing)
            ) {
              const convertedTahapanKerja = convertBoqPricingToTahapanKerja(
                fallbackData.boqPricing
              );
              ...
              setBoqTitle(
                `${fallbackData.vendorName} - ${fallbackData.projectTitle} BOQ`
              );
              setTahapanKerja(convertedTahapanKerja);
              ...
              setTimeout(() => {
                try {
                  ...
                  console.warn(
                    "⚠️ Failed to cleanup fallback data:",
                    cleanupError
                  );
                }
              }, 5000);
        ...
        throw new Error(
          "Temporary BOQ data not found in localStorage. The data may have expired or been removed."
        );
      }

      const tempBOQData = JSON.parse(tempBOQDataString);
      console.log(
        "✅ Temporary BOQ data loaded from localStorage:",
        tempBOQData
      );
      ...
      if (tempBOQData.boqPricing && Array.isArray(tempBOQData.boqPricing)) {
        ...
        setTimeout(() => {
          try {
            ...
            console.warn(
              "⚠️ Failed to cleanup localStorage data:",
              cleanupError
            );
          }
      ...
      console.error(
        "❌ Error loading temporary BOQ data from localStorage:",
        error
      );
  ...
  useEffect(() => {
    ...
    if (
      (tempId || localKey || sessionKey) &&
      (mode === "view" || mode === "resubmission")
    ) {
      ...
      if (sessionKey) {
        ...
        console.log(
          "📱 Attempting to load from Firestore with tempId:",
          tempId
        );
        ...
        console.log(
          "💾 Attempting to load from localStorage with localKey:",
          localKey
        );
    ...
    if (
      ...
      if (autosaveData) {
        try {
          ...
          if (
            hoursSinceAutosave < 24 &&
            (draft.boqTitle.trim() ||
              draft.tahapanKerja.some((t) => t.name.trim()))
          ) {
  ...
  const loadSessionBOQData = (sessionKey, isReadOnly) => {
    try {
      console.log(
        "📥 Loading session BOQ data from localStorage...",
        sessionKey
      );
      ...
      if (!sessionDataString) {
        console.warn(
          "⚠️ Session BOQ data not found in localStorage with key:",
          sessionKey
        );
        throw new Error(
          "Session BOQ data not found. The data may have expired or been removed."
        );
      }
      ...
      if (sessionData.boqPricing && Array.isArray(sessionData.boqPricing)) {
        ...
        console.log(
          "🔍 First few BOQ items structure:",
          sessionData.boqPricing.slice(0, 3).map((item, index) => ({
            index,
        ...
        const displayAmount =
          sessionData.finalAmount || sessionData.totalAmount;
        ...
        console.log(
          ...
          sessionData.showPriceComparison
        );
        ...
        setTimeout(() => {
          try {
            ...
            console.warn(
              "⚠️ Failed to cleanup session data from localStorage:",
              cleanupError
            );
          }
        ...
        throw new Error(
          "Session BOQ data structure is invalid or missing pricing information"
        );
      }
    } catch (error) {
      console.error(
        "❌ Error loading session BOQ data from localStorage:",
        error
      );
  ...
  const loadResubmissionBOQData = async (sessionKey) => {
    try {
      console.log(
        "📥 Loading resubmission BOQ data from localStorage for project info...",
        sessionKey
      );
      ...
      if (!sessionDataString) {
        throw new Error(
          "Resubmission session data not found. Please try resubmitting the proposal again."
        );
      }
      ...
      if (!sessionData.projectId || !sessionData.vendorId) {
        throw new Error(
          "Invalid resubmission session data - missing project or vendor ID"
        );
      }

      // Fetch the latest project data directly from Firestore
      console.log(
        "🔄 Fetching latest project data from Firestore...",
        sessionData.projectId
      );
      const latestProject = await firestoreService.get(
        "projects",
        sessionData.projectId
      );
      ...
      if (latestProject.proposals && Array.isArray(latestProject.proposals)) {
        proposalIndex = latestProject.proposals.findIndex(
          (proposal) => proposal.vendorId === sessionData.vendorId
        );

        if (proposalIndex !== -1) {
          vendorProposal = latestProject.proposals[proposalIndex];
          console.log(
            "✅ Vendor proposal found at index:",
            proposalIndex,
            vendorProposal
          );
        }
      ...
      console.log(
        "🔍 Full vendor proposal data:",
        JSON.stringify(vendorProposal, null, 2)
      );
      console.log(
        "🔍 Full project data for negotiation check:",
      ...
      if (
        vendorProposal.negotiation &&
        Object.keys(vendorProposal.negotiation).length > 0
      ) {
        latestNegotiationData = vendorProposal.negotiation;
        console.log(
          "📍 Found negotiation data in vendor proposal:",
          latestNegotiationData
        );
      }
      // Check project-level negotiations
      else if (
        latestProject.negotiations &&
        Array.isArray(latestProject.negotiations)
      ) {
        const vendorNegotiation = latestProject.negotiations.find(
          (neg) => neg.vendorId === sessionData.vendorId
        );
        if (vendorNegotiation) {
          latestNegotiationData = vendorNegotiation;
          console.log(
            "📍 Found negotiation data in project negotiations:",
            latestNegotiationData
          );
        }
      }
      // Check project-level negotiation object
        ...
        latestProject.negotiation &&
        Object.keys(latestProject.negotiation).length > 0
      ) {
        latestNegotiationData = latestProject.negotiation;
        console.log(
          "📍 Found negotiation data in project negotiation:",
          latestNegotiationData
        );
      }
      // Check if negotiation data is embedded in proposal status
        ...
        vendorProposal.negotiationHistory
      ) {
        const latestNegotiation =
          vendorProposal.negotiationHistory[
            vendorProposal.negotiationHistory.length - 1
          ];
        if (latestNegotiation) {
          latestNegotiationData = latestNegotiation;
          console.log(
            "� Found negotiation data in negotiation history:",
            latestNegotiationData
          );
        }
      }
      ...
      console.log(
        "📋 BOQ pricing detailed structure:",
        JSON.stringify(existingBOQPricing, null, 2)
      );
      ...
      if (existingBOQPricing.length > 0) {
        ...
        const recreatedTahapanKerja = existingBOQPricing.map((item, index) => {
          ...
          if (
            latestNegotiationData.boqPricing &&
            latestNegotiationData.boqPricing[index]
          ) {
            negotiatedPrice =
              latestNegotiationData.boqPricing[index].negotiatedPrice ||
              latestNegotiationData.boqPricing[index].pricePerPcs ||
              latestNegotiationData.boqPricing[index].unitPrice ||
              originalVendorPrice;
          } else if (
            latestNegotiationData.negotiatedItems &&
            latestNegotiationData.negotiatedItems[index]
          ) {
            negotiatedPrice =
              latestNegotiationData.negotiatedItems[index].negotiatedPrice ||
          ...
          console.log(
            `💰 Item ${index} (${
              item.itemDescription || item.name || "Unknown"
            }):`,
            {
              originalVendorPrice,
              negotiatedPrice,
                    ...
                    spec: [
                      {
                        ...
                        name:
                          item.specName || item.specification || "Spesifikasi",
        ...
        console.log(
          "✅ BOQ data recreated from existing proposal with original prices:",
          recreatedTahapanKerja
        );
      } else {
        console.log(
          "⚠️ No existing BOQ pricing found, starting with default structure"
        );
      }
      ...
      console.log(
        "✅ Resubmission BOQ data loaded successfully from Firestore"
      );
    } catch (error) {
  ...
  const addBulkJenisKerja = (tahapanId, count) => {
    console.log(
      "addBulkJenisKerja called with tahapanId:",
      tahapanId,
      "count:",
      count,
      "type of count:",
    ...
    setTahapanKerja((prevTahapan) =>
      prevTahapan.map((tahapan) =>
        tahapan.id === tahapanId
          ? {
              ...tahapan,
              jenisKerja: [...tahapan.jenisKerja, ...newJenisKerja],
            }
          : tahapan
      )
    );
  ...
  const addBulkUraian = (tahapanId, jenisId, count) => {
    console.log(
      "addBulkUraian called with tahapanId:",
      tahapanId,
      "jenisId:",
      jenisId,
      "count:",
  ...
  const handleContextMenu = (e, tahapanId, jenisId = null, uraianId = null) => {
    ...
    console.log("handleContextMenu called with:", {
      tahapanId,
      jenisId,
      uraianId,
    });
    ...
    if (e.key === "Enter") {
      ...
      setTimeout(() => {
        ...
        if (lastUraian) {
          ...
          const uraianInputs = document.querySelectorAll(
            `input[data-uraian-id="${lastUraian.id}"]`
          );
  ...
  const handleDrop = (e, targetTahapanId) => {
    ...
    const newTahapanKerja = [...tahapanKerja];
    const draggedIndex = newTahapanKerja.findIndex(
      (t) => t.id === draggedTahapan
    );
    const targetIndex = newTahapanKerja.findIndex(
      (t) => t.id === targetTahapanId
    );
  ...
  const handleJenisDrop = (e, targetJenisId, tahapanId) => {
    ...
    const draggedIndex = tahapan.jenisKerja.findIndex(
      (j) => j.id === draggedJenis
    );
    const targetIndex = tahapan.jenisKerja.findIndex(
      (j) => j.id === targetJenisId
    );
    ...
    setTahapanKerja(
      tahapanKerja.map((tahapan) =>
        tahapan.id === tahapanId
          ? {
              ...tahapan,
              jenisKerja: tahapan.jenisKerja.filter(
                (jenis) => jenis.id !== jenisId
              ),
            }
          : tahapan
      )
          ...
          ? {
              ...
              jenisKerja: tahapan.jenisKerja.map((jenis) =>
                jenis.id === jenisId
                  ? {
                      ...jenis,
                      uraian: jenis.uraian.filter(
                        (uraian) => uraian.id !== uraianId
                      ),
                    }
                  : jenis
              ),
  ...
  const exportToCSV = () => {
    ...
    csvContent +=
      "Tahapan Kerja,Jenis Pekerjaan,Uraian,Spesifikasi,Volume,Satuan,Harga per Pcs,Total\n";
            ...
            jenis.uraian.forEach((uraian) => {
              if (uraian.spec.length === 0) {
                ...
                uraian.spec.forEach((spec) => {
                  ...
                  csvContent += `"${tahapan.name}","${jenis.name}","${
                    uraian.name
                  }","${spec.description}",${spec.volume},"${
                    spec.satuan
                  }","Rp ${spec.pricePerPcs.toLocaleString(
                    "id-ID"
    ...
    link.setAttribute(
      "download",
      `BOQ_${boqTitle.replace(/[^a-z0-9]/gi, "_").toLowerCase()}_${
  ...
  const saveBOQ = () => {
    ...
    const boqData = {
      ...
      createdAt: currentBOQId
        ? getSavedBOQ(currentBOQId)?.createdAt
  ...
  const handleResubmitUpdatedProposal = async () => {
    ...
    if (calculateGrandTotal() === 0) {
      alert(
        "BOQ tidak boleh kosong. Silakan tambahkan minimal satu item dengan harga."
      );
      return;
    }
    ...
    try {
      ...
      if (updatedBOQPricing.length === 0) {
        alert(
          "Tidak ada item BOQ dengan harga dan volume yang valid. Silakan lengkapi data BOQ terlebih dahulu."
        );
        return;
      }
      ...
      const updatedProposal = {
        ...
        resubmissionNotes: `Proposal telah diperbarui berdasarkan feedback project owner. Total item: ${
          updatedBOQPricing.length
        }, Total harga: Rp ${totalAmountWithPPN.toLocaleString("id-ID")}`,
        previousTotalAmount:
          resubmissionData.originalProposal?.totalAmount || 0,
        negotiationHistory:
          resubmissionData.originalProposal?.negotiationHistory || [],
        // Ensure these critical fields are preserved
        submittedAt:
          resubmissionData.originalProposal?.submittedAt ||
          new Date().toISOString(),
        submittedBy:
          resubmissionData.originalProposal?.submittedBy ||
          resubmissionData.vendorId,
        userId:
          resubmissionData.originalProposal?.userId ||
          resubmissionData.vendorId,
        message: resubmissionData.originalProposal?.message || "",
        negotiable:
          resubmissionData.originalProposal?.negotiable || "negotiable",
        // Update timestamps
        updatedAt: new Date().toISOString(),
      };
                ...
                {savedBOQs.length === 0 ? (
                  <div className="text-center py-24">
                    ...
                    <p className="text-gray-500 mb-8 max-w-md mx-auto">
                      Mulai dengan membuat Bill of Quantities pertama Anda. Anda
                      dapat menyimpan, mengedit, dan mengelola semua BOQ dari
                      sini.
                    </p>
                    <button
                      onClick={createNewBOQ}
                  ...
                  <div>
                    <div className="mb-6">
                      <h2 className="text-lg font-medium text-gray-900">
                        BOQ Anda ({savedBOQs.length})
                      </h2>
                      <p className="text-sm text-gray-500">
                        Kelola dan atur Bill of Quantities Anda
                      </p>
                    </div>
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                  <div>
                    ...
                    <p className="text-blue-100">
                      Buat Bill of Quantities detail dengan penyimpanan lokal
                    </p>
                  </div>
                      ...
                      <button
                        onClick={() =>
                          (window.location.href =
                            returnUrl || "/dashboard/project-owner")
                        }
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <h3 className="text-lg font-semibold text-amber-800">
                        Mode Pengajuan Ulang Proposal
                      </h3>
                      <p className="text-sm text-amber-700">
                        Anda sedang mengedit proposal Anda berdasarkan masukan
                        pemilik proyek. Harga sebelumnya ditampilkan untuk
                        perbandingan.
                      </p>
                    </div>
                  </div>
                  ...
                  {lastAutoSave && (
                    <div className="mt-2 flex items-center space-x-2 text-sm text-green-600">
                      ...
                      <span>
                        ...
                        {lastAutoSave.toLocaleTimeString("id-ID", {
                          hour: "2-digit",
                          minute: "2-digit",
                          second: "2-digit",
                        })}
                    ...
                    <div className="w-full">
                      <table className="w-full table-fixed">
                        <thead className="bg-gray-100 sticky top-0 z-10">
                          <tr>
                            <th
                              className="px-4 py-3 text-left text-sm font-bold text-gray-800 border-r border-gray-300"
                              style={{
                                width: showPriceComparison ? "15%" : "18%",
                              }}
                            >
                              Tahapan Kerja
                            </th>
                            ...
                            <th
                              className="px-4 py-3 text-left text-sm font-bold text-gray-800 border-r border-gray-300"
                              style={{
                                width: showPriceComparison ? "13%" : "16%",
                              }}
                            >
                              Spesifikasi
                            </th>
                            ...
                            <th
                              className="px-4 py-3 text-left text-sm font-bold text-gray-800 border-r border-gray-300"
                              style={{
                                width: showPriceComparison ? "12%" : "14%",
                              }}
                            >
                              Harga
                            </th>
                            <th
                              className="px-4 py-3 text-left text-sm font-bold text-gray-800"
                              style={{
                                width: showPriceComparison ? "14%" : "16%",
                              }}
                            >
                              Total
                            </th>
                        ...
                        <tbody>
                          {tahapanKerja.map((tahapan, tahapanIndex) => {
                            ...
                            const rows = [];
                            const isLastTahapan =
                              tahapanIndex === tahapanKerja.length - 1;
                            ...
                            if (tahapan.jenisKerja.length === 0) {
                              ...
                              tahapan.jenisKerja.forEach(
                                (jenis, jenisIndex) => {
                                  const isLastJenis =
                                    jenisIndex ===
                                    tahapan.jenisKerja.length - 1;
                                  ...
                                  if (jenis.uraian.length === 0) {
                                    ...
                                    jenis.uraian.forEach(
                                      (uraian, uraianIndex) => {
                                        const isLastUraian =
                                          uraianIndex ===
                                          jenis.uraian.length - 1;
                                        ...
                                        rows.push({
                                          ...
                                          uraianIndex,
                                          spec:
                                            uraian.spec.length > 0
                                              ? uraian.spec[0]
                                              : null, // Use first (and only) spec
                                          specId:
                                            uraian.spec.length > 0
                                              ? uraian.spec[0].id
                                              : null,
                            ...
                            return rows.map((row, rowIndex) => {
                              ...
                              const isFirstJenisRow =
                                rows.findIndex(
                                  (r) => r.jenisId === row.jenisId
                                ) === rowIndex;
                              // Check if this is the first row for this uraian
                              const isFirstUraianRow =
                                rows.findIndex(
                                  (r) => r.uraianId === row.uraianId
                                ) === rowIndex;
                              // Check if this is the first row for this tahapan
                              const isFirstTahapanRow =
                                rows.findIndex(
                                  (r) => r.tahapanId === row.tahapanId
                                ) === rowIndex;

                              // Calculate rowspan for tahapan (how many rows belong to this tahapan)
                              const tahapanRowSpan = rows.filter(
                                (r) => r.tahapanId === row.tahapanId
                              ).length;
                              // Calculate rowspan for jenis (how many rows belong to this jenis)
                              const jenisRowSpan = rows.filter(
                                (r) => r.jenisId === row.jenisId && r.jenisId
                              ).length;
                            ...
                            `}
                                ...
                                `}
                                        ...
                                        <div className="flex items-center space-x-2">
                                          ...
                                          {editMode && (
                                            <button
                                              onClick={(e) => {
                                                console.log(
                                                  "Jenis context menu clicked, row data:",
                                                  {
                                    ...
                                    <td className="px-4 py-2 border-r border-gray-200 group cursor-pointer"></td>
                                  ) : null}
                            ...
                            `}
                                  ...
                                  >
                                    {row.spec ? (
                                      <input
                                        ...
                                        min="0"
                                        value={
                                          row.spec.volume === null ||
                                          row.spec.volume === undefined
                                            ? ""
                                            : row.spec.volume
                                        }
                                        onChange={(e) => {
                                  ...
                                  {showPriceComparison && (
                                    <td className="px-4 py-2 border-r border-gray-200 align-middle">
                                      {row.spec &&
                                      (row.spec.hasOriginalPrice ||
                                        row.spec.originalPrice > 0) ? (
                                  ...
                                  {/* Harga */}
                                  <td
                                    className="px-4 py-2 border-r border-gray-200 align-middle cursor-not-allowed"
                                    onClick={(e) => {
                          ...
                          <tr className="border-t-2 border-gray-300 bg-white">
                            <td
                              colSpan={showPriceComparison ? "8" : "7"}
                          ...
                          <tr className="bg-white">
                            ...
                            <td className="px-4 py-3 font-medium text-gray-600">
                              Rp{" "}
                              {(calculateGrandTotal() * 0.11).toLocaleString(
                                "id-ID"
                              )}
                            </td>
                          </tr>
                          <tr className="border-t border-gray-300 bg-white">
                            <td
                              colSpan={showPriceComparison ? "8" : "7"}
                              className="px-4 py-4 text-right font-bold text-black text-lg"
                            >
                              Total:
                            </td>
                            <td className="px-4 py-4 font-bold text-black text-lg">
                              Rp{" "}
                              {(calculateGrandTotal() * 1.11).toLocaleString(
                                "id-ID"
                              )}
                            </td>
                          </tr>
                      ...
                      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                        <div className="text-sm text-gray-600">
                          <p className="font-medium">
                            Ready to resubmit your updated proposal?
                          </p>
                          <p>
                            Total Amount:{" "}
                        ...
                        <div className="flex gap-3">
                          ...
                          <button
                            ...
                            disabled={
                              !boqTitle.trim() || calculateGrandTotal() === 0
                            }
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                            <div>
                              ...
                              <span className="ml-2 font-medium">
                                {resubmissionData.latestProject?.projectTitle}
                              </span>
                            </div>
                            <div>
              ...
              <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-md mx-4 border border-gray-200">
                <div className="text-center">
                  ...
                  <p className="text-gray-700 mb-8 leading-relaxed">
                    Your Bill of Quantities has been saved. Ready to take the
            ...
            <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50">
              <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-md mx-4 border border-gray-200">
                <div className="text-center">
                  ...
                  <h3 className="text-2xl font-bold text-black mb-3">
                    Tambah Banyak Tahapan Kerja
                  </h3>
                  <p className="text-gray-700 mb-6 leading-relaxed">
                    Berapa banyak tahapan kerja yang ingin Anda tambahkan?
                  </p>
            ...
            <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50">
              <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-md mx-4 border border-gray-200">
                <div className="text-center">
                  ...
                  <h3 className="text-2xl font-bold text-black mb-3">
                    Tambah Banyak Jenis Pekerjaan
                  </h3>
                  <p className="text-gray-700 mb-6 leading-relaxed">
                    Berapa banyak jenis pekerjaan yang ingin Anda tambahkan?
                  </p>

                  <div className="mb-6">
                    <label
                      htmlFor="bulkJenisCount"
                      className="block text-sm font-medium text-black mb-3"
                    >
                  ...
                  <div className="space-y-3">
                    <button
                      onClick={() => {
                        console.log(
                          "Button clicked - bulkAddJenisTahapanId:",
                          bulkAddJenisTahapanId,
                          "bulkAddJenisCount:",
                          bulkAddJenisCount
                        );
                        addBulkJenisKerja(
            ...
            <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50">
              <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-md mx-4 border border-gray-200">
                <div className="text-center">
                  ...
                  <h3 className="text-2xl font-bold text-black mb-3">
                    Tambah Banyak Uraian
                  </h3>
                  <p className="text-gray-700 mb-6 leading-relaxed">
                    Berapa banyak uraian yang ingin Anda tambahkan?
                  </p>

                  <div className="mb-6">
                    <label
                      htmlFor="bulkUraianCount"
                      className="block text-sm font-medium text-black mb-3"
                    >
                      Jumlah Uraian (1-50)
                    </label>
                    <input
                      ...
                      onKeyPress={(e) => {
                        ...
                        if (e.key === "Enter") {
                          addBulkUraian(
                            bulkAddUraianTahapanId,
                            bulkAddUraianJenisId,
                            bulkAddUraianCount
                          );
                  ...
                  <div className="space-y-3">
                    <button
                      onClick={() => {
                        console.log(
                          "Uraian Button clicked - bulkAddUraianTahapanId:",
                          bulkAddUraianTahapanId,
                          "bulkAddUraianJenisId:",
                          bulkAddUraianJenisId,
                          "bulkAddUraianCount:",
